...
class Test:
    ...
    def startTest(self):
        ...
        if current_url == "https://juice-shop.herokuapp.com/#/login":
            fr.log("Login page reached, test was able to register an user using an invalid password, test failed.")
            ...
            return fr.getReturnData(False, f"[✖] ({test_name}) test was able to register an user using an invalid password, test failed.")
        else:
            fr.log("Login page not reached, assuming no user was created using invalid password, test passed.")
            fr.log("Credentials used:")
            fr.log(f"    User email: {user_email}")
            fr.log(f"    User password: {user_password}")
            fr.log("Stopping test.")
